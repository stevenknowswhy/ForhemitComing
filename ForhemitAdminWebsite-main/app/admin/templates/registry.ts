...
export interface TemplateFormHandle {
  ...
  /** Returns the form's root DOM node for PDF rendering (fallback for html2canvas) */
  getContainerRef: () => HTMLDivElement | null;
  /**
   * Optional: If the form provides its own PDF generator (e.g. jsPDF-based),
   * the modal will call this instead of html2canvas. Returns void when done.
   */
  generatePDF?: () => Promise<void>;
  /**
   * Optional: If the form needs special printing behavior (e.g. printing only
   * the contract DOM, not the data-entry UI), implement this method.
   */
  printContract?: () => void;
}
...
export const templateFormRegistry: TemplateFormEntry[] = [
  {
    ...
    component: () => import("./forms/engagement-letter"),
  },
  {
  ...
  },
  {
   ...
   },
   {
     ...
     label: "Transition Stewardship Agreement",
     component: () => import("./forms/stewardship-agreement"),
   },
   {
     formKey: "engagement-letter-standalone",
     label: "Engagement Letter (Standalone)",
     component: () => import("./forms/engagement-letter/StandaloneEngagementLetterForm"),
   },
   {
     formKey: "stewardship-agreement-standalone",
     label: "Transition Stewardship Agreement (Standalone)",
     component: () => import("./forms/stewardship-agreement/StandaloneStewardshipAgreementForm"),
   },
   // ───────── Add new forms below ─────────
  // {
