...
async function logContentEvent(
	...
	await logEvent(ctx, {
		...
		eventType:
			action === "published"
				? LOG_ACTIONS.SYSTEM_PUBLISHED
				: LOG_ACTIONS.SYSTEM_UPDATED,
		category: "system",
		summary: `Content ${action}: ${title}`,
		source: "admin_ui",
		visibility: "internal",
		scopeType: "system",
		entityType: "post",
...
export const listPublished = query({
	...
	handler: async (ctx, args) => {
		...
		return rows
			.filter((p) => p.publishedAt !== undefined && p.publishedAt <= now)
			.slice(0, lim);
	},
});
...
export const create = mutation({
	...
	handler: async (ctx, args) => {
		...
		// Log to business log if published
		if (status === "published") {
			await logContentEvent(ctx, "published", args.title, slug, id);
		}
	...
	handler: async (ctx, args) => {
		...
		await ctx.db.patch(args.id, {
			...
			...(args.ogImage !== undefined
				? {
						ogImage: args.ogImage === null ? undefined : args.ogImage.trim(),
					}
				: {}),
			...
			...(args.depthLevel !== undefined
				? {
						depthLevel: args.depthLevel === null ? undefined : args.depthLevel,
					}
				: {}),
		...
		// Log to business log if status changed to published
		if (nextStatus === "published" && post.status !== "published") {
			await logContentEvent(ctx, "published", post.title, nextSlug, args.id);
		} else if (nextStatus === "published") {
			await logContentEvent(ctx, "updated", post.title, nextSlug, args.id);
		}
...
export const publish = mutation({
	...
	handler: async (ctx, args) => {
		...
		await logContentEvent(ctx, "published", post.title, post.slug, args.id);
