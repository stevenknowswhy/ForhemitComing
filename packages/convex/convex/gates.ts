...
export const setGate = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(args.companyId, { gates: updated, updatedAt: Date.now() });
    return updated;
  },
});
...
export const setGates = mutation({
  args: {
    companyId: v.id("crmCompanies"),
    gates: v.object({
      gate1: v.object({ passed: v.boolean(), passedAt: v.optional(v.number()) }),
      gate2: v.object({ passed: v.boolean(), passedAt: v.optional(v.number()) }),
      gate3: v.object({ passed: v.boolean(), passedAt: v.optional(v.number()) }),
      gate4: v.object({ passed: v.boolean(), passedAt: v.optional(v.number()) }),
    }),
  },
  handler: async (ctx, args) => {
    const company = await ctx.db.get(args.companyId);
 ...
 */
export const calculateGateTriggerDueDate = (gateState: GateState | undefined, daysOffset: number): number | undefined => {
 ...
 */
export const checkGateRequirements = (gates: GatesState | undefined, requiredGate?: string): boolean => {
