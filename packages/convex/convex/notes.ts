  ...
  handler: async (ctx, args) => {
    ...
    const enriched = await Promise.all(
      notes.map(async (note) => {
        ...
        const contact = note.contactId ? await ctx.db.get(note.contactId) : null;
        const authorData = author as any;
        const contactData = contact as any;
        return {
          ...note,
          authorName: authorData ? `${authorData.firstName || ""} ${authorData.lastName || ""}`.trim() || authorData.email : "Unknown",
          contactName: contactData ? `${contactData.firstName} ${contactData.lastName}` : null,
        };
      })
    );
...
export const getAllNotes = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.type) {
      notes = await ctx.db.query("notes")
    ...
    const enriched = await Promise.all(
      notes.map(async (note) => {
        const author = note.authorId ? await ctx.db.get(note.authorId) : null;
        const company = note.companyId ? await ctx.db.get(note.companyId) : null;
        const authorData = author as any;
        return {
          ...note,
          authorName: authorData ? `${authorData.firstName || ""} ${authorData.lastName || ""}`.trim() || authorData.email : "Unknown",
