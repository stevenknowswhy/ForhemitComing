...
export const transitionDealStage = mutation({
	...
	handler: async (
		...
		for (const req of requirements) {
			...
			if (
				contactIds.length === 0 &&
				req.requiredForAudience.includes("internal")
			) {
				// Check for duplicate
				...
				if (shouldCreate) {
					...
					if (req.recurrenceRule) {
						await ctx.db.insert("workflowTasks", {
							...
							contactId: undefined,
							direction:
								template.category === "communication" ? "outbound" : "inbound",
							status: "pending",
		...
		await ctx.runMutation(api.journalEntries.autoLog, {
			...
			entryType: "milestone",
			title: `Stage Advanced: ${oldStage} → ${newStage}`,
			description: `Deal moved from "${oldStage}" to "${newStage}".`,
			clientDescription: `Your transaction has advanced to the ${newStage} phase.`,
			performedBy: "system",
			autoGeneratedFrom: "stage_transition",
		});

		// Journal chapter management — close current chapter, open new one
		let closedChapterId: string | undefined;
		let journalIdForCloseSummary: string | undefined;
		try {
			const journal: { _id: string; chapterNumber: number } | null =
				await ctx.runQuery(api.clientJournals.getByClient, {
			...
			console.error("Chapter management failed:", chapterErr);
		}
		...
		return {
			...
			closedChapterId,
			journalId: journalIdForCloseSummary,
		};
	},
});
...
export const markTaskCompleted = mutation({
	...
	handler: async (ctx, args) => {
		...
		await ctx.runMutation(api.journalEntries.autoLog, {
			companyId: task.companyId,
			entryType: "work",
			title: `Task Completed: ${templateTitle}`,
			description: `Task "${templateTitle}" marked as completed.`,
			clientDescription: `${templateTitle} has been completed.`,
			performedBy: "system",
			relatedTaskId: task._id,
			autoGeneratedFrom: "task_completed",
		});
	...
	handler: async (ctx) => {
		...
		result.sort(
			(a, b) =>
				b.missing.length +
				b.overdue.length -
				(a.missing.length + a.overdue.length),
		);
...
// ============================================
// triggerCloseSummary — generate chapter close summary PDF (action, can call runAction)
// ============================================

export const triggerCloseSummary = action({
	args: {
	...
	handler: async (
		ctx,
		args,
	): Promise<{ success: boolean; boxFileId?: string; error?: string }> => {
		const result = (await ctx.runAction(
			internal.journalPdf.generateCloseSummary,
			{
				journalId: args.journalId,
				chapterId: args.chapterId,
			},
		)) as { success?: boolean; boxFileId?: string; error?: string };

		if (result.error) {
			return { success: false, error: result.error };
		}

		return { success: true, boxFileId: result.boxFileId };
	},
});
