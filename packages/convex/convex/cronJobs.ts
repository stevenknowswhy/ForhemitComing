...
export const createRecurringTasks = internalMutation({
	...
	handler: async (ctx) => {
		...
		const recurringParents = await ctx.db
			.query("workflowTasks")
		...
		for (const parent of recurringParents) {
			...
			if (parent.status === "completed" || parent.status === "skipped")
				continue;
			...
			const needsNewInstance =
				...
				(() => {
					...
					const threshold =
						RECURRENCE_THRESHOLDS[parent.recurrenceRule ?? ""] ?? Infinity;
...
export const checkTimeBasedTriggers = internalMutation({
	...
	handler: async (ctx) => {
		...
		const timeBasedReqs = allReqs.filter(
			(r) =>
				r.trigger &&
				[
					"DAYS_FROM_STAGE",
		...
		for (const company of activeDeals) {
			...
			for (const req of matches) {
				...
				// Calculate dueDate
				const dueDate = calculateTriggerDueDate(
					req as TriggerRequirement,
					company as DealCompany,
					now,
				);
