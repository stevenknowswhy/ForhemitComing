...
export const update = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (updates.description !== undefined)
			typedUpdates.description = updates.description;
		if (updates.dueDate !== undefined) typedUpdates.dueDate = updates.dueDate;
		if (updates.status)
			typedUpdates.status = updates.status as Doc<"crmTasks">["status"];
		if (updates.priority)
			typedUpdates.priority = updates.priority as Doc<"crmTasks">["priority"];
		if (updates.assignedTo !== undefined)
			typedUpdates.assignedTo = updates.assignedTo;
