  ...
  handler: async (ctx, args) => {
    ...
    const matches = triggered.filter((req) => {
      ...
      if (args.event === "gate_passed") {
        return req.trigger === "ON_GATE_PASSED" && req.triggerGate === args.gateName;
      }
      if (args.event === "time_check") {
        if (req.trigger === "DAYS_FROM_STAGE" && company.stageEnteredAt) {
          const dueDate = calculateStageTriggerDueDate(company.stageEnteredAt, req.daysOffset ?? 0);
        ...
        if (req.trigger === "DAYS_FROM_GATE" && req.triggerGate) {
          ...
          const dueDate = calculateGateTriggerDueDate(gate, req.daysOffset ?? 0);
          return dueDate !== undefined && dueDate <= now;
        }
      }
      return false;
 ...
 */
export const validateTriggerConfig = (trigger?: TriggerType, triggerGate?: string, daysOffset?: number): string[] => {
  const errors: string[] = [];
