...
export async function sendAndLogEmail(
	...
	opts?: {
		...
		relatedCompanyId?: Id<"crmCompanies">;
		relatedContactId?: Id<"crmContacts">;
	},
): Promise<{ success: boolean; error?: string; id?: string }> {
	...
	if (result.success) {
		try {
			await ctx.runMutation((await import("./emailEvents")).logOutbound, {
				from: payload.from || FROM_EMAIL,
		...
		// Journal entry for email sent
		if (opts?.relatedCompanyId) {
			try {
				await ctx.runMutation((await import("./journalEntries")).autoLog, {
					companyId: opts.relatedCompanyId,
					entryType: "email",
					title: `Email Sent: ${payload.subject}`,
					description: `Email sent to ${Array.isArray(payload.to) ? payload.to.join(", ") : payload.to}. Subject: ${payload.subject}`,
					clientDescription: `Email communication sent: ${payload.subject}`,
					touchpointType: "email",
					performedBy: "system",
					autoGeneratedFrom: "email_sent",
				});
			} catch (journalErr) {
				console.error("Failed to create journal entry for email:", journalErr);
			}
		}
	}
