...
export const generateDocument = action({
	...
	handler: async (
		...
		try {
			...
			const filledFilename = emailConfig.filename.replace(
				/\{\{(\w+)\}\}/g,
				(_match, key: string) => {
			...
			// 5. Format email body
			const filledBody = emailConfig.body.replace(
				/\{\{(\w+)\}\}/g,
				(_match, key: string) => {
					const value = data[key];
					return value ? value : "unknown";
			...
			const emailResult: any = await ctx.runAction(
				api.templateEmailer.sendTemplateEmail,
				{
					to: recipientEmail,
...
export const testTemplateRendering = action({
	...
	handler: async (ctx, args): Promise<any> => {
		...
		// Load template
		const template: any = await ctx.runQuery(api.templates.getByTitle, {
			title: templateTitle,
		});
...
export const previewEmailConfig = action({
	...
	handler: async (ctx, args) => {
		...
		const filledSubject = emailConfig.subject.replace(
			/\{\{(\w+)\}\}/g,
			(_match, key: string) => {
				const value = data[key];
				return value ? value : "unknown";
		...
		return {
			subject: filledSubject,
			body: filledBody,
			filename: emailConfig.filename.replace(
				/\{\{(\w+)\}\}/g,
				(_match, key: string) => {
					const value = data[key];
					return value ? value : "unknown";
