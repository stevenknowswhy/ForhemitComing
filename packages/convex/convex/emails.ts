...
&copy; ${new Date().getFullYear()} Forhemit. All rights reserved.
    `.trim();
		...
		const telegramResult = await sendTelegramMessage(
			buildContactSubmissionTelegramText(args),
		);
...
export const sendJobApplicationNotification = action({
	...
	handler: async (_ctx, args) => {
		...
		const positionDisplay =
			args.position === "Other" && args.otherPosition
				? args.otherPosition
				: args.position;
	...
	handler: async (_ctx, args) => {
		...
		const detailRows = [
			...
			{ label: "Entity Type", value: args.entity },
			{
				label: "Timeline",
				value: args.timeline
					? timelineLabels[args.timeline] || args.timeline
					: undefined,
			},
...
export const sendClassificationIntakeNotification = action({
	...
	handler: async (_ctx, args) => {
		...
		try {
			parsed = JSON.parse(args.answers);
		} catch {
			/* empty */
		}
