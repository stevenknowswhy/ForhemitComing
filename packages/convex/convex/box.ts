...
export const handleBoxWebhook = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (
			...
			issueEvents.includes(args.eventType)
		) {
			const template = await ctx.db.get(task.templateId);
			const templateTitle = template?.title || "Document";
			const isSignature = signatureEvents.includes(args.eventType);

			await ctx.runMutation(api.journalEntries.autoLog, {
				companyId: task.companyId,
				entryType: isSignature ? "signature" : "issue",
				title: isSignature
					? `Document Signed: ${templateTitle}`
					: `Signature ${args.eventType === "SIGN_REQUEST.DECLINED" ? "Declined" : "Expired"}: ${templateTitle}`,
				description: isSignature
					? `${templateTitle} signed via Box Sign. Request: ${args.signRequestId}`
					: `${templateTitle} signature ${args.eventType === "SIGN_REQUEST.DECLINED" ? "declined" : "expired"}. Request: ${args.signRequestId}`,
				clientDescription: isSignature
					? `${templateTitle} has been signed by all parties.`
					: undefined,
				touchpointType: "document",
				performedBy: "box-sign",
				relatedTaskId: task._id,
				visibleToClient: isSignature,
				sensitivity: isSignature ? "low" : "medium",
				internalNote: isSignature
					? undefined
					: `Box Sign ${args.eventType} — ${args.signRequestId}`,
				autoGeneratedFrom: `box_${args.eventType.toLowerCase().replace("sign_request.", "")}`,
			});
		}
...
// ============================================
// uploadFileToFolder — upload a file to a specific Box folder
// Used by journal PDF generation to upload the weekly digest.
// ============================================

export const uploadFileToFolder = internalAction({
	args: {
		folderId: v.string(),
		fileName: v.string(),
		content: v.array(v.number()),
	},
	handler: async (_ctx, args) => {
		const bytes = new Uint8Array(args.content);
		const file = await uploadFile(args.fileName, bytes, args.folderId);
