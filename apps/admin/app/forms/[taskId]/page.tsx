...
export default function FormPage() {
  ...
  const formData = useQuery(api.formSubmissions.getFormData, { workflowTaskId: taskId });
  ...
  if (formData.task.status === "received" || formData.task.status === "completed") {
    return (
      <div className="min-h-screen flex items-center justify-center bg-stone-50">
        <div className="max-w-md w-full bg-white rounded-lg shadow-sm border border-stone-200 p-8 text-center">
          <div className="text-4xl mb-4">✓</div>
          <h1 className="text-xl font-semibold text-stone-900 mb-2">Form Already Submitted</h1>
    ...
    <div className="min-h-screen bg-stone-50 py-12 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-t-lg shadow-sm border border-stone-200 border-b-0 p-6">
          ...
          <h1 className="text-2xl font-semibold text-stone-900">{formData.template?.title}</h1>
          <p className="text-stone-600 mt-2">{formData.template?.description}</p>

          {formData.company && (
            <div className="mt-4 pt-4 border-t border-stone-100">
              <p className="text-sm text-stone-500">
                Company: <span className="text-stone-700 font-medium">{formData.company.name}</span>
              </p>
              <p className="text-sm text-stone-500">
                Stage: <span className="text-stone-700">{formData.company.stage}</span>
              </p>
            </div>
          )}
        </div>
            ...
            <div className="space-y-4 mb-6">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={formData.contact ? `${formData.contact.firstName} ${formData.contact.lastName}` : "Your name"}
                />
              </div>
