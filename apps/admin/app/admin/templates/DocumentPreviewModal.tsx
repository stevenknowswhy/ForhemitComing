...
import {
	...
	exportToJSON,
	exportToExcel,
} from "@forhemit/shared/lib/export-utils";
...
export default function DocumentPreviewModal({
	...
	const isMobileDevice = useCallback(() => {
		...
		const userAgent = /mobile|android|iphone|ipad|ipod/i.test(
			navigator.userAgent,
		);
	...
	const handleExportCSV = useCallback(async () => {
		...
		const entry = formKey ? getFormEntry(formKey) : null;
		const prefix =
			entry?.label?.replace(/\s+/g, "-") ?? templateName.replace(/\s+/g, "-");
	...
	return (
		<div className="template-modal-overlay" onClick={onClose}>
			...
			>
				...
				{generationError && (
					<div
						className="pdf-generation-error"
						style={{
							padding: "12px 24px",
							background: "#fee2e2",
							color: "#991b1b",
				...
				<div className="template-modal-body" data-pdf="true">
					{FormComponent ? (
						<Suspense
							fallback={<div className="templates-loading">Loading form…</div>}
						>
							<FormComponent ref={formRef} initialData={initialFormData} />
						</Suspense>
					) : (
