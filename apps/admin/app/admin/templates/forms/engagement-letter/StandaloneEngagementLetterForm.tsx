...
function applyTitle(doc: Document, title: string) {
	...
	const custom = doc.getElementById(
		"f-of-title-custom",
...
function mapTailToSelect(value: unknown): string {
	...
	if (value.includes("90") && !value.includes("180") && !value.includes("270"))
		return "90";
...
function scrapeFormData(doc: Document | null): Record<string, unknown> {
	...
	const chk = (id: string) =>
		(doc.getElementById(id) as HTMLInputElement | null)?.checked ?? false;
	...
	const officerTitle =
		sel?.value === "_other" ? (custom?.value ?? "") : (sel?.value ?? "");
	...
	return {
		...
		brokerFirstName: v("f-bfn"),
		brokerLastName: v("f-bln"),
		brokerFirm: v("f-bf"),
...
function applyInitialData(doc: Document, raw: Record<string, unknown>) {
	...
	if (typeof raw.officer_name === "string")
		setVal(doc, "f-of-name", raw.officer_name);
	applyTitle(
		doc,
		typeof raw.officer_title === "string" ? raw.officer_title : "",
	);
	...
	if (typeof raw.retainer_date === "string")
		setVal(doc, "f-pd", raw.retainer_date);
	...
	if (typeof raw.venue_county === "string")
		setVal(doc, "f-ac", raw.venue_county);
	...
	if (typeof raw.special_terms === "string")
		setVal(doc, "f-ad", raw.special_terms);

	doc
		.querySelectorAll(".cp-body input, .cp-body select, .cp-body textarea")
		.forEach((el) => {
...
>(function StandaloneEngagementLetterForm({ initialData }, ref) {
	...
	useImperativeHandle(
		...
		() => ({
			getFormData: () =>
				scrapeFormData(iframeRef.current?.contentDocument ?? null),
			getContainerRef: () =>
