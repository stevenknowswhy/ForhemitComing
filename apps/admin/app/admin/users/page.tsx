...
export default function UsersManagementPage() {
	...
	const fetchUsers = async () => {
		try {
			...
			if (!response.ok) {
				if (
					data.message?.includes("Configure Clerk Dashboard") ||
					data.message?.includes("session token")
				) {
					...
					<div className="flex items-start gap-4">
						...
						<div>
							...
							<p className="text-[var(--text-secondary)] mb-6">
								The session token needs to be configured in your Clerk Dashboard
								to include the email claim.
							</p>
							...
							<div className="space-y-4">
								...
								<div className="bg-[var(--bg-secondary)] rounded-lg p-4">
									...
									<p className="text-[var(--text-secondary)]">
										Go to <strong>Sessions</strong> →{" "}
										<strong>Customize session token</strong>
									</p>
								</div>
								...
								<div className="bg-[var(--bg-secondary)] rounded-lg p-4">
									...
									<p className="text-[var(--text-secondary)] mb-2">
										Add this to your session token template:
									</p>
  ...
  "name": "{{user.full_name}}"
								...
								<div className="bg-[var(--bg-secondary)] rounded-lg p-4">
									...
									<p className="text-[var(--text-secondary)]">
										1. Click <strong>Save</strong> in Clerk Dashboard
										<br />
										2. Sign out and sign back in to this admin panel
										<br />
										3. Refresh this page
									</p>
								</div>
			...
			<div className="space-y-6">
				...
				<section className="space-y-3 rounded-xl bg-[#f2f4f4] p-3 md:flex md:items-center md:gap-3 md:space-y-0">
					<AdminSearchInput
						...
						className="flex-1"
						inputClassName="rounded-xl border-none bg-white dark:bg-[#2A3028] text-sm ring-0 placeholder:text-[#8d9495] focus:ring-1 focus:ring-[#4c635d]/40"
				...
				{loading ? (
					...
					<>
						<section className="md:hidden">
							<div className="overflow-hidden rounded-xl bg-white dark:bg-[#2A3028] shadow-sm dark:shadow-none ring-1 ring-[#adb3b4]/10">
									...
									>
										<div className="flex min-w-0 flex-1 items-center gap-3">
											...
											<div className="min-w-0">
												...
												<p className="truncate text-xs text-[#5a6061]">
													{u.email}
												</p>
											</div>
										</div>
										{u.role !== "super-admin" ? (
											<details className="users-mobile-row-menu relative shrink-0">
												...
												<div className="absolute right-0 top-full z-10 mt-1 min-w-[9rem] rounded-xl border border-[#e5e7eb] bg-white dark:bg-[#2A3028] py-1 shadow-lg">
													<button
														type="button"
						...
						<section className="hidden overflow-hidden rounded-xl bg-white dark:bg-[#2A3028] md:block">
							...
							{filteredUsers.map((u) => (
								<div
									key={u.id}
									className="grid grid-cols-[2.2fr_1fr_1fr_1fr_80px] items-center px-6 py-4 hover:bg-[#f9fbfb]"
								>
									<div className="flex min-w-0 items-center gap-3">
										...
										<div className="min-w-0">
											...
											<p className="truncate text-sm text-[#5a6061]">
												{u.email}
											</p>
										</div>
									</div>
						...
						{filteredUsers.length === 0 && (
							<div className="rounded-xl bg-white dark:bg-[#2A3028] p-10 text-center">
					...
					<div className="flex flex-wrap items-center gap-2">
						...
						{showExtraPagination ? (
							<>
								<button
									type="button"
									className="h-9 w-9 rounded-lg border border-[#e5e7eb] bg-white dark:bg-[#2A3028] text-sm font-semibold text-[#5a6061]"
								>
									2
								</button>
								<button
						...
						<form onSubmit={handleInvite}>
							...
							<div className="modal-footer-stitch">
								...
								<button
									type="submit"
									disabled={
										inviteLoading || !inviteEmail.endsWith("@forhemit.com")
									}
									className="btn-stitch btn-stitch-primary"
								>
									{inviteLoading ? (
