...
import { usePartnerCRM } from "@forhemit/shared/features/esop-partners";
	...
	const contactCounts = useMemo(
		() => ({
			...
			byType: {
				...
				Administrator: contacts.filter((c) => c.type === "Administrator")
					.length,
	...
	return (
		<div className="flex min-h-screen bg-[#F2F5F9]">
			...
			<div className="flex-1 flex flex-col min-w-0">
				{/* Topbar */}
				<div className="bg-white border-b border-[#E8EDF5] px-4 sm:px-6 min-h-[3.25rem] py-2 flex flex-wrap items-center gap-2 sm:gap-3 sticky top-0 z-50">
					<h1 className="font-serif text-base text-[#0E1C2F] min-w-0 flex-1 basis-full sm:basis-auto sm:flex-1">
					...
					<AdminSearchInput
						...
						className="min-w-0 w-full sm:w-48 sm:flex-initial sm:max-w-xs"
						inputClassName="rounded-full border border-[#E8EDF5] bg-[#F2F5F9] py-2 text-xs text-[#0E1C2F] focus:border-[#1A5FA8] focus:outline-none"
						iconLeftClassName="left-3 h-3.5 w-3.5 text-[#8298AE]"
						clearButtonClassName="text-[#8298AE] hover:bg-[#E8EDF5]/80 hover:text-[#0E1C2F]"
					/>
							...
							<>
								...
								{hasNudges && (
									<div className="bg-[#FEF3DC] border border-[#E8C97A] rounded-lg px-4 py-2.5 mb-4 flex items-center gap-2.5">
										...
										<span className="text-xs text-[#C47A0A]">
											<strong>
												{metrics.nudge} contact{metrics.nudge > 1 ? "s" : ""}
											</strong>{" "}
											overdue — {nudgeNames}
								...
								<div className="flex items-center gap-2 mb-4 flex-wrap">
									{["all", ...STAGE_ORDER].map((stage) => {
										...
										const isActive =
											(stage === "all" &&
												!(STAGE_ORDER as readonly string[]).includes(
													currentView,
												)) ||
											currentView === stage;
										...
										<button
											...
											className={`px-2.5 py-1 text-[11px] flex items-center gap-1 transition-colors ${
												cardView === "grid"
													...
													: "bg-white text-[#8298AE] hover:bg-[#F2F5F9]"
											}`}
										>
			...
			<DetailPanel
				...
				onMarkStepDone={(stepId) =>
					selectedContact && markJourneyStepDone(selectedContact.id, stepId)
				}
				onMarkDocSent={(stepId) =>
					selectedContact && markJourneyDocSent(selectedContact.id, stepId)
				}
				onUndoStep={(stepId) =>
					selectedContact && undoJourneyStep(selectedContact.id, stepId)
				}
				onAddNote={(text) =>
					selectedContact && addNote(selectedContact.id, text)
				}
				onDeleteNote={(noteId) =>
					selectedContact && deleteNote(selectedContact.id, noteId)
				}
				onAddActivity={(text, type) =>
					selectedContact && addActivity(selectedContact.id, text, type)
				}
				onDeleteActivity={(activityId) =>
					selectedContact && deleteActivity(selectedContact.id, activityId)
				}
				onAdvanceStage={() =>
					selectedContact && advanceStage(selectedContact.id)
				}
			/>
