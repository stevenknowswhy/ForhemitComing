...
interface GhostStats {
	...
	total: number;
	error_count: number;
	by_type: { type: string; count: number; last_generated: string }[];
}
...
export default function AdminDashboardPage() {
	...
	useEffect(() => {
		fetch("/api/ghost/stats")
			.then((r) => r.json())
			.then(setGhostStats)
			.catch(() => setGhostStats(null));
	}, []);
		...
		<div>
			...
			<section style={{ marginTop: "2rem" }}>
				...
				{!ghostStats ? (
					...
					>
						<div
							style={{ display: "flex", gap: "2rem", marginBottom: "1.25rem" }}
						>
							<div>
								<span
									style={{
							...
							<div>
								<span
									style={{
										...
										fontWeight: 600,
										color:
											ghostStats.error_count > 0
												? "#ef4444"
												: "var(--text-primary)",
									}}
								>
									{ghostStats.error_count}
								</span>
								<span
									style={{
										fontSize: "0.8125rem",
