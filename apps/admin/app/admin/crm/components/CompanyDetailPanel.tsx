...
import ClientIntroductionLetter from "../../letters/components/ClientIntroductionLetter";
import PreliminaryReviewLetter from "../../letters/components/PreliminaryReviewLetter";
...
export function CompanyDetailPanel({
	...
	const [activeLetter, setActiveLetter] = useState<"intro" | "review" | null>(
		null,
	);
		...
		<>
			<div className="crm-detail-panel crm-animate-slide-in">
				...
				<div className="crm-detail-header">
					<div>
						<h2 className="text-2xl font-semibold mb-1.5 text-[var(--text)]">
							{company.name}
						</h2>
				...
				<div className="crm-detail-body">
					...
					<Section title="Stage & Status">
						<div className="flex gap-2 flex-wrap mb-3">
							...
							<span
								className={`crm-nda-badge ${
									company.ndaStatus === "Signed"
										? "crm-nda-signed"
										: company.ndaStatus === "Pending"
											? "crm-nda-pending"
											: "crm-nda-none"
								}`}
							>
								...
								value={
									primaryContact?.email ? (
										<a
											...
											className="text-[var(--blue)] no-underline hover:underline"
										>
											{primaryContact.email}
										</a>
									) : (
						...
						<Section title="Next Step">
							{company.nextStep && (
								<div className="bg-[var(--surface2)] border border-[var(--border)] rounded-md p-3">
									<div className="text-[var(--text)] mb-1.5 text-[14px]">
										{company.nextStep}
									</div>
							...
							{company.expectedCloseDate && (
								<div className="bg-[var(--surface2)] border border-[var(--border)] rounded-md p-3 mt-2">
									...
									<div className="text-[var(--text)] text-[14px]">
										{company.expectedCloseDate}
									</div>
								</div>
							)}
					...
					<Section title="Interaction Log">
						<InteractionLog companyId={companyId} />
					</Section>
					...
					<Section title="Letters">
						<div className="flex gap-2">
							<button
								onClick={() => setActiveLetter("intro")}
								className="flex-1 btn btn-ghost text-[12px]"
							>
								📝 Intro Letter
							</button>
							<button
								onClick={() => setActiveLetter("review")}
								className="flex-1 btn btn-ghost text-[12px]"
							>
								📊 Preliminary Review
							</button>
			...
			{activeLetter && (
				...
				>
					<div
						className="w-full max-w-[1200px] max-h-[90vh] overflow-auto rounded-xl shadow-2xl"
						onClick={(e) => e.stopPropagation()}
					>
						{activeLetter === "intro" ? (
							<ClientIntroductionLetter
								companyId={companyId}
								companyStage={company.stage}
							/>
						) : (
							<PreliminaryReviewLetter
								companyId={companyId}
								companyStage={company.stage}
							/>
						)}
...
function Field({ label, value }: { label: string; value: React.ReactNode }) {
	return (
		<div>
			...
			<span className="text-[13px] text-[var(--text)] break-words">
				{value}
			</span>
		</div>
	);
...
function TimelineItem({ activity }: { activity: Doc<"crmActivities"> }) {
	...
	return (
		<div
			className={`flex gap-4 py-3 border-l-2 border-[var(--border)] pl-5 relative ${borderColors[activity.type] || ""}`}
		>
			<div
