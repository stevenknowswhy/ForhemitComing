...
export function DealQueueView() {
	...
	return (
		<div className="space-y-4">
			{activeDeals.map((company: DealCompany) => (
...
function DealCard({ company }: { company: DealCompany }) {
		...
		<div className="bg-[var(--surface)] border border-[var(--border)] rounded-lg overflow-hidden">
			...
			<div className="flex items-center justify-between px-4 py-3 bg-[var(--surface2)] border-b border-[var(--border)]">
				...
				<div className="flex items-center gap-3">
					<span className="text-[12px] text-[var(--text2)]">
						{company.fees?.totalFee
							? formatCurrency(company.fees.totalFee)
							: "—"}
					</span>
			...
			{pending.length > 0 && (
				<div className="px-4 py-2">
					...
					{pending.slice(0, 10).map((task: DealTask) => (
						<TaskRow
							key={task.taskId}
							task={task}
							variant="pending"
					...
					>
						...
						<div className="flex gap-2 mt-6">
							...
							<button
								...
								disabled={
									!recipientEmail || !recipientName || generating !== null
								}
					...
					>
						...
						<div className="space-y-3">
							...
							<p className="text-[10px] text-[var(--text3)]">
								Box Sign will email the signer a link to review and sign the
								document.
							</p>
						</div>
						<div className="flex gap-2 mt-6">
...
function TaskRow({
	...
	const signStatus = task.boxSignStatus
		? signStatusMap[task.boxSignStatus]
		: null;
	...
	return (
		...
		>
			...
			<div className="flex items-center gap-2 shrink-0">
				...
				{signStatus ? (
					<span
						className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${signStatus.color}`}
					>
						{signStatus.label}
					</span>
				) : (
