...
import { type Company, type PipelineStage, STAGE_STYLES } from "@forhemit/shared/features/crm";
...
function KanbanCard({ company, onClick }: KanbanCardProps) {
  ...
  if (company.nextStepDate) {
    if (days !== null && days < 0) {
      dueIndicator = (
        <span className="text-[9px] text-[var(--red)]">⚠ {Math.abs(days)}d overdue</span>
      );
    } else if (days === 0) {
      dueIndicator = <span className="text-[9px] text-[var(--amber-dim)]">◉ Due today</span>;
    } else if (days !== null && days <= 3) {
      dueIndicator = <span className="text-[9px] text-[var(--green-dim)]">◎ {days}d</span>;
    }
  }
      ...
      <div className="flex items-center justify-between mt-2 pt-2 border-t border-[var(--border)]">
        <span
          className={`
            ...
            ${company.ndaStatus === "Signed" ? "bg-emerald-100 text-emerald-900" : ""}
            ${company.ndaStatus === "Pending" ? "bg-amber-100 text-amber-900" : ""}
