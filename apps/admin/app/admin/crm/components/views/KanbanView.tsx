...
export function KanbanView({ companies, onSelect }: KanbanViewProps) {
	...
	return (
		<div className="flex-1 overflow-x-auto overflow-y-hidden p-3 sm:p-5 flex gap-3 sm:gap-4 bg-[var(--bg)]">
			{STAGES.map((stage) => (
				<KanbanColumn
					key={stage}
					stage={stage}
...
function KanbanColumn({ stage, companies, onSelect }: KanbanColumnProps) {
	...
	return (
		<div className="min-w-[280px] max-w-[280px] flex flex-col max-h-full rounded-lg border border-[var(--border)] bg-[var(--surface)]">
			{/* Header */}
			<div
...
function KanbanCard({ company, onClick }: KanbanCardProps) {
	...
	if (company.nextStepDate) {
		if (days !== null && days < 0) {
			dueIndicator = (
				<span className="text-[9px] text-[var(--red)]">
					⚠ {Math.abs(days)}d overdue
				</span>
			);
		} else if (days === 0) {
			dueIndicator = (
				<span className="text-[9px] text-[var(--amber-dim)]">◉ Due today</span>
			);
		} else if (days !== null && days <= 3) {
			dueIndicator = (
				<span className="text-[9px] text-[var(--green-dim)]">◎ {days}d</span>
			);
		}
	}
	...
	return (
		<div
			onClick={onClick}
			className="bg-[var(--surface)] border border-[var(--border)] rounded-md p-3 cursor-pointer
		...
		>
			...
			{/* Next Step */}
			{company.nextStep && (
				<div className="text-[11px] text-[var(--text2)] my-2">
					{company.nextStep}
				</div>
			)}
            ...
            ${company.ndaStatus === "Signed" ? "bg-green-100 text-green-900 dark:bg-green-900/30 dark:text-green-300" : ""}
            ${company.ndaStatus === "Pending" ? "bg-amber-100 text-amber-900 dark:bg-amber-900/30 dark:text-amber-300" : ""}
