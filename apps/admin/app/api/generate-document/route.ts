		...
		if (body.companyId) {
			try {
				...
				if (convexUrl) {
					...
					await fetch(`${siteUrl}/log-journal-entry`, {
						method: "POST",
						headers: { "Content-Type": "application/json" },
						body: JSON.stringify({
							companyId: body.companyId,
							entryType: "document",
							title: `Document Generated: ${templateTitle}`,
							description: `${templateTitle} generated and ${boxFileId ? "uploaded to Box" : "prepared"}.`,
							clientDescription: `${templateTitle} has been prepared for your review.`,
							touchpointType: "document",
							performedBy: "admin-api",
							relatedTaskId: body.taskId,
							relatedDocuments: boxFileId ? [boxFileId] : undefined,
							autoGeneratedFrom: "document_generated",
						}),
					});
				}
			} catch (journalErr) {
				console.error("Journal entry failed:", journalErr);
				// Don't fail the request for journal issues
			}
		}
