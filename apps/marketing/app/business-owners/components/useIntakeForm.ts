...
const STEP_REQUIREMENTS: Record<string, (form: Record<string, string>, agreed: boolean, signed: boolean, path?: string | null) => boolean> = {
...
export function useIntakeForm(defaultPath?: "nda" | "light" | null) {
  ...
  const setField = useCallback(
    (k: string, v: string) => setState((s) => ({ ...s, form: { ...s.form, [k]: v } })),
    [],
  );
  ...
  const canAdvance = useCallback(
    (currentStepId: string | undefined): boolean => {
      ...
      return check ? check(state.form, state.agreed, state.signed, state.path) : true;
    },
  ...
  }, [state.path, state.form, submitContact, sendIntakeNotification, buildMessage]);
