...
export function InfrastructureAuditModal({
	isOpen,
	onClose,
}: InfrastructureAuditModalProps) {
	...
	const sendNotification = useAction(
		api.emails.sendInfrastructureAuditNotification,
	);

	if (!isOpen) return null;

	const currentScore = Object.values(answers).reduce(
		(sum, val) => sum + (val || 0),
		0,
	);
...
Diagnostic: ${diagnostic}`,
	...
	const questions = [
		{
			...
			description:
				"Does your business demonstrate 5+ years of structural integrity (consistent profitability with stable cash flow)?",
			options: [
				{
					value: 20,
					label: "Yes - 5+ years of consistent, stable profitability",
					points: "20 pts",
				},
				{
					value: 10,
					label: "Partial - 3-5 years with minor volatility",
					points: "10 pts",
				},
				{
					value: 0,
					label: "No - Less than 3 years or inconsistent cash flow",
					points: "0 pts",
				},
			],
		},
		{
			...
			description:
				"What is the current capacity of your Human Grid (total full-time employees)? ESOPs typically require critical mass for tax efficiency.",
			options: [
				{
					value: 20,
					label: "About 20 FTEs (optimal for ESOP mechanics)",
					points: "20 pts",
				},
				{
					value: 10,
					label: "10-19 FTEs (Viable but less efficient)",
					points: "10 pts",
				},
				{
					value: 5,
					label: "Under 10 FTEs (Below critical mass)",
					points: "5 pts",
				},
			],
		},
		{
			...
			description:
				"If you initiated a 3-Month Controlled Shutdown (zero contact), would operational systems maintain autonomous function?",
			options: [
				{
					value: 20,
					label: "Yes - The business runs without my daily input",
					points: "20 pts",
				},
				{
					value: 10,
					label:
						"Partial - Major decisions still require me, but operations continue",
					points: "10 pts",
				},
				{
					value: 0,
					label: "No - I am the load-bearing wall; systems fail without me",
					points: "0 pts",
				},
			],
		},
		{
			...
			description:
				"Does your average employee tenure exceed 5 years? (High tenure proves there is legacy worth protecting through ownership)",
			options: [
				{ value: 20, label: "Yes - 5+ years average tenure", points: "20 pts" },
				{
					value: 10,
					label: "Mixed - 3-5 years, some turnover",
					points: "10 pts",
				},
				{
					value: 0,
					label: "No - High turnover, mostly under 3 years",
					points: "0 pts",
				},
			],
		},
		{
			...
			description:
				"Are critical business processes documented in SOPs, or do they exist only in your head (tribal knowledge)?",
			options: [
				{
					value: 20,
					label:
						"Documented - Critical knowledge is written, accessible, and trainable",
					points: "20 pts",
				},
				{
					value: 10,
					label: "Partial - Some documentation, but gaps in key areas",
					points: "10 pts",
				},
				{
					value: 0,
					label:
						"Tribal - I am the primary repository of critical business knowledge",
					points: "0 pts",
				},
			],
		},
	...
	const currentQuestion =
		currentStep > 0 && currentStep <= 5 ? questions[currentStep - 1] : null;
				...
				<div className="audit-modal-content">
					...
					<div className="audit-header">
						...
						<div
							...
							style={{ opacity: currentScore > 0 ? 1 : 0.3 }}
						>
									...
									<div className="audit-step">
										<div className="step-header-large">
											...
											<h3 className="step-title-large">
												Select Your Infrastructure Path
											</h3>
										</div>
										<p className="step-description-large">
											This determines what we build together. Choose the lane
											that matches your current strategic position.
										</p>
										...
										<div className="lane-grid-large">
											...
											>
												<span className="lane-title-large">
													Lane 1: Resilience
												</span>
												<span className="lane-desc-large">
													I plan to keep running my business, but need a
													failsafe infrastructure if I can no longer operate.
												</span>
											</button>
											...
											>
												...
												<span className="lane-desc-large">
													I intend to exit completely and transfer 100%
													ownership to my employees within 3–5 years.
												</span>
											</button>
											...
											>
												...
												<span className="lane-desc-large">
													I intend to sell to a third party, but want a credible
													ESOP alternative to maximize negotiating power.
												</span>
											</button>
										</div>
									</div>
										...
										<div className="options-grid-large">
											{currentQuestion.options.map((opt) => (
												<button
													...
													onClick={() =>
														handleAnswer(currentQuestion.key, opt.value)
													}
												>
													<div className="option-radio">
														<div
															className={`radio-dot ${answers[currentQuestion.key] === opt.value ? "active" : ""}`}
														/>
													</div>
													<div className="option-content">
														<span className="option-label-large">
															{opt.label}
														</span>
													</div>
													<span className="option-points-large">
														{opt.points}
													</span>
												</button>
											))}
									...
									<>
										...
										<div className="results-header-compact">
											...
											<div className="results-score-row">
												<div className={`score-display ${results.status}`}>
													{results.score}
												</div>
												...
												<div className="tab-panel summary-panel">
													<div className="summary-cta">
														<p className="summary-intro">
															Your infrastructure score of{" "}
															<strong>{results.score}/100</strong> indicates
													...
													<div
														...
														dangerouslySetInnerHTML={{
															__html: DOMPurify.sanitize(
																getDiagnosticText(lane, results.status),
															),
														}}
													/>
												</div>
