...
export function TableView({
  ...
  if (companies.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[400px] text-[var(--text3)]">
        <span className="text-5xl mb-4 opacity-30">🔍</span>
        <h3 className="font-serif text-lg text-[var(--text)] mb-2">No companies found</h3>
        <p className="text-[13px]">Try adjusting your filters or add a new company</p>
      </div>
    );
  }
  ...
  return (
    <th
      ...
      className={`
        ...
        transition-colors
        ${isActive ? "text-[#2dd882]" : "text-[var(--text3)] hover:text-[var(--text2)]"}
      `}
    >
      {label}
      ...
      <td className="px-3.5 py-3">
        <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            ...
            className="px-2 py-1 text-[11px] bg-[var(--surface)] border border-[var(--border)] rounded text-[var(--text3)] hover:text-[#2dd882] hover:border-[#1a8f52] transition-colors"
          >
            Edit
          </button>
          <button
