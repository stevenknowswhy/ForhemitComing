...
export function CalendarView({ companies, onSelect, onAddWithDate }: CalendarViewProps) {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-5 bg-[var(--bg)]">
      <div className="grid grid-cols-7 gap-2 max-w-[1200px] mx-auto">
        {/* Header */}
  ...
  return (
    <div
      ...
      className={`
        ...
        ${isToday ? "border-[#1a8f52] bg-[#0d4a2a]" : "border-[var(--border)] bg-[var(--surface)] hover:border-[var(--border2)]"}
      `}
    >
      <div className={`text-sm font-semibold mb-2 ${isToday ? "text-[#2dd882]" : "text-[var(--text)]"}`}>
        {day}
      </div>
      ...
      <div className="flex flex-col gap-1">
        {companies.map((company) => {
          ...
          let borderLeftColor = "#2dd882"; // Future
          if (days !== null && days < 0) borderLeftColor = "#ff5f5f"; // Overdue
          else if (days === 0) borderLeftColor = "#f5a623"; // Today
          ...
          return (
            <div
              ...
              className="bg-[var(--bg)] border border-[var(--border2)] rounded px-1.5 py-1 text-[10px]
                cursor-pointer truncate hover:border-[#1a8f52] hover:bg-[var(--surface2)] transition-all"
