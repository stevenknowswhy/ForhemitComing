...
export function AnalyticsView({ stats }: AnalyticsViewProps) {
  ...
  // Prepare NDA data
  const ndaDist = stats.ndaDistribution ?? { Signed: 0, Pending: 0, None: 0 };
  const ndaData = [
    { name: "Signed", count: ndaDist.Signed, color: "#2dd882" },
    { name: "Pending", count: ndaDist.Pending, color: "#f5a623" },
    { name: "None", count: ndaDist.None, color: "#94a3b8" },
  ].filter((d) => d.count > 0);
