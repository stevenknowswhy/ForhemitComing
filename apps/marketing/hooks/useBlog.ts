...
export function usePathwayProvider() {
	...
	useEffect(() => {
		...
		if (
			stored &&
			["all", "founders", "attorneys", "lenders", "cpas", "employees"].includes(
				stored,
			)
		) {
		...
		if (
			roleParam &&
			["founders", "attorneys", "lenders", "cpas", "employees"].includes(
				roleParam,
			)
		) {
...
export function usePathway() {
	...
	if (!ctx) {
		throw new Error(
			"usePathway must be used within a PathwayProvider. Wrap your blog layout with <PathwayProvider>.",
		);
	}
	return ctx;
}
