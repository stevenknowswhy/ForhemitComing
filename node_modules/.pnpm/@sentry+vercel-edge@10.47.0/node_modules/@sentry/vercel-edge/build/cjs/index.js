...
function parsePairKeyValue(entry) {
    ...
    if (metadataSeparatorIndex !== -1 &&
        ...
        metadata = api.baggageEntryMetadataFromString(metadataString);
    }
...
function getNumberFromEnv(key) {
    ...
    if (isNaN(value)) {
        api.diag.warn(`Unknown value ${inspect(raw)} for ${key}, expected a number, using defaults`);
        return undefined;
    }
    return value;
}
...
class SpanImpl {
    ...
    addLink(link) {
        ...
        if (linkCountLimit !== undefined && this.links.length >= linkCountLimit) {
            if (this._droppedLinksCount === 0) {
                api.diag.debug('Dropping extra links.');
            }
            this.links.shift();
            this._droppedLinksCount++;
        }
    ...
    setStatus(status) {
        ...
        if (status.code === api.SpanStatusCode.UNSET)
            return this;
        if (this.status.code === api.SpanStatusCode.OK)
            return this;
        ...
        if (status.code === api.SpanStatusCode.ERROR) {
    ...
    updateName(name) {
        ...
        if (this._droppedLinksCount > 0) {
            api.diag.warn(`Dropped ${this._droppedLinksCount} links because linkCountLimit reached`);
        }
    ...
    _isSpanEnded() {
        if (this._ended) {
            ...
            api.diag.warn(`Cannot execute the operation on ended Span {traceId: ${this._spanContext.traceId}, spanId: ${this._spanContext.spanId}}`, error);
        }
        return this._ended;
    }
...
class ParentBasedSampler {
    ...
    shouldSample(context, traceId, spanName, spanKind, attributes, links) {
        const parentContext = api.trace.getSpanContext(context);
...
class TraceIdRatioBasedSampler {
    ...
    shouldSample(context, traceId) {
        return {
            decision: api.isValidTraceId(traceId) && this._accumulate(traceId) < this._upperBound
...
function buildSamplerFromEnv() {
    ...
    switch (sampler) {
        ...
        default:
            api.diag.error(`OTEL_TRACES_SAMPLER value "${sampler}" invalid, defaulting to "${TracesSamplerValues.ParentBasedAlwaysOn}".`);
            return new ParentBasedSampler({
                root: new AlwaysOnSampler(),
            });
    }
...
function getSamplerProbabilityFromEnv() {
    ...
    if (probability < 0 || probability > 1) {
        api.diag.error(`OTEL_TRACES_SAMPLER_ARG=${probability} was given, but it is out of range ([0..1]), defaulting to ${DEFAULT_RATIO}.`);
        return DEFAULT_RATIO;
    }
    return probability;
}
...
class BasicTracerProvider {
    ...
    constructor(config = {}) {BasicTracerProvider.prototype.__init.call(this);
        ...
        this._resource = mergedConfig.resource ?? resources.defaultResource();
...
class SentryPropagator extends W3CBaggagePropagator {
   ...
   */
   inject(context, carrier, setter) {
    if (isTracingSuppressed(context)) {
      DEBUG_BUILD$1 && core.debug.log('[Tracing] Not injecting trace data for url because tracing is suppressed.');
      return;
    }
  ...
  /** @inheritDoc */
   shouldSample(
    ...
    if (method === 'OPTIONS' || method === 'HEAD') {
      DEBUG_BUILD$1 && core.debug.log(`[Tracing] Not sampling span because HTTP method is '${method}' for ${spanName}`);
    ...
    if (
      ...
      DEBUG_BUILD$1 && core.debug.log('[Tracing] Discarding root span because its trace was not chosen to be sampled.');
...
function getParentSampled(parentSpan, traceId, spanName) {
  ...
  if (api.isSpanContextValid(parentContext) && parentContext.traceId === traceId) {
    ...
    DEBUG_BUILD$1 && core.debug.log(`[Tracing] Inheriting parent's sampled decision for ${spanName}: ${parentSampled}`);
    return parentSampled;
  }
