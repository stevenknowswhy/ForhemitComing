...
function appRouterInstrumentPageLoad(client) {
  const pathname = stripTrailingSlash(react.WINDOW.location.pathname);
  const parameterizedPathname = parameterization.maybeParameterizeRoute(pathname);
...
function appRouterInstrumentNavigation(client) {
  routerTransitionHandler = (href, navigationType) => {
    ...
    const unparameterizedPathname = stripTrailingSlash(new URL(normalizedHref, react.WINDOW.location.href).pathname);
