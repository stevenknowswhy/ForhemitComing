import { escapeStringForRegex, parseSemver, loadModule, debug } from '@sentry/core';
import * as fs from 'fs';
import { createRequire } from 'module';
import * as path from 'path';
import { normalizePathForGlob, getBuildPluginOptions } from './getBuildPluginOptions.js';
import { getNextjsVersion, getPackageModules } from './util.js';
...
function constructWebpackConfigFunction({
  ...
  return function newWebpackFunction(
    ...
    if (!(isDev || (isStaticExport && isServer))) {
      ...
      const { sentryWebpackPlugin } = loadModule('@sentry/webpack-plugin', module) ?? {};
      ...
      if (sentryWebpackPlugin) {
        ...
        const distDir = normalizePathForGlob((userNextConfig ).distDir ?? '.next');
...
export { constructWebpackConfigFunction };
